...
A minimal native macOS interface for [Pi](https://pi.dev): one persistent Pi session per conversation tab.

## Run

1. Install and authenticate Pi.
2. Open `Nauvis.xcodeproj` and run the app.
3. Press `⌘T` for a new session and `⌘W` to close one. Click a tool call to inspect it in a new split.
